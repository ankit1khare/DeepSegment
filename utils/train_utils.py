...
def add_softmax_cross_entropy_loss_for_each_scale(scales_to_logits,
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    # not_ignore_mask = sum(tf.to_float(tf.equal(scaled_labels, c['index'])) * c['weight'] for c in CLASS_INFO.values())
    not_ignore_mask = []
    for c in CLASS_INFO.values():
      not_ignore_mask.append(tf.to_float(tf.equal(scaled_labels, c['index'])) * c['weight'])
    not_ignore_mask = sum(not_ignore_mask)
